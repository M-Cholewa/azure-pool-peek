...
def main(timer_trigger1: func.TimerRequest) -> None:
    ...
    logging.info('SCRAPPER: Python timer trigger function started at %s', utc_timestamp)

    try:
        # 1. Scrapowanie Strony
        POOL_URL = "https://gosirbochnia.pl/api/index.php"
        logging.info("SCRAPPER: Pobieranie danych z URL: %s", POOL_URL)
        ...
        if counter_element:
            ...
            if people_count_str:
                ...
                logging.error("SCRAPPER: Nie znaleziono liczby w tagu <h3>. Surowy tekst: %s", raw_text)
                return
        else:
            logging.error("SCRAPPER: Nie znaleziono tagu <h3> na stronie.")
            return
        ...
        if not connection_string:
            logging.error("SCRAPPER: Brak zmiennej środowiskowej POOL_STORAGE_CONNECTION.")
            return
        ...
        logging.info("SCRAPPER: Sukces! Zapisano dane: Liczba osób: %d", people_count)

    except Exception as e:
        logging.error("SCRAPPER: Krytyczny błąd podczas scrapowania/zapisu: %s", str(e))